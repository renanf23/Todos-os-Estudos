  ...
  switch (option) {
    case 1:
      ...
      const crewLimit = Number(
        prompt("Insira a quantidade maxima da tripulação")
      );
    ...
    case 2:
      const shipName = prompt(
        "Digite o nome da nave que você deseja adiconar o tripulante:"
      );
...
function findsapaceShip(shipName: string) {
  let ship: {
    name: string;
    pilot: string;
    crewLimit: number;
...
function newCrew(
  crewName: string,
  spaceShip: { name: string; pilot: string; crewLimit: number; crew: string[] }
) {
