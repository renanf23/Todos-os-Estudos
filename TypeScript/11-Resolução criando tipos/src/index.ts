...
function updateSituation(situation: PlanetSituation, planet: Planet) {
  ...
  alert(`A situação do ${planet.name} foi atualizada para ${situation}.`);
}
...
function promptValiPlanet(callbackfn: (planet: Planet) => void) {
...
function firstMenuOption() {
  ...
  const confimation = confirm(`Confirma o registro do planeta ${name}?
    Coordnadas: (${coordinateA}, ${coordinateB}, ${coordinateC}, ${coordinateD})
    Situação: ${situation}`);
  if (confimation) {
    addPlanet(
      name,
