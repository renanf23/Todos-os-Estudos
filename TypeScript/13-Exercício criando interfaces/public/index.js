const users = [];
async function firstMenuOption(userName) {
...
function promptValidUser(name, callbackfn) {
...
function secondMenuOption(name) {
...
function fifthMenuOption() {
    ...
    for (let i = 0; i < users.length; i++) {
        rank += `${i + 1} - ${users[i].name} com ${users[i].publicRepos} repositórios.\n`;
    }
    console.log(rank);
}
