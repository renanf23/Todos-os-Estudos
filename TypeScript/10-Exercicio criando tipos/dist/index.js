...
function firstMenuOption() {
    ...
    if (planetVerify) {
        alert(`Esse planeta já existe, por favor use um nome de planeta diferente.`);
    }
    else {
        let cord = [];
        for (let i = 0; i < 4; i++) {
...
function createPlanet(name, coords, situation) {
    const planet = {
        name,
        coords,
        situation,
...
function situationatt(planet) {
    const option = Number(prompt(`A situação atual do planeta ${planet.name} é ${planet.situation}\nPara mudar digite : 1 - Habitado\n 2 - Habitável\n 3 - Inabitável\n 4 - Inexplorado`));
    if (option == 1) {
        planet.situation = "Habitado";
        alert(`A situação do planeta ${planet.name} mudou para ${planet.situation}`);
    }
    else if (option == 2) {
...
function secondMenuOption() {
    const planetName = prompt("Insira o nome do planeta que deseja atualizar a situação:");
    const planetVerify = findPlanet(planetName);
    if (planetVerify) {
